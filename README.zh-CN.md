...
bootstrapper 抛出的全部报错都跟随 `--language`，包括嵌入的失败原因，诊断路径与安装路径同样适用。当某个下游命令失败时，报错会附带该命令自身输出的末尾若干行，因此根因不会被吞掉。这一点由测试保证：新增的抛出点若没有英文变体，测试会失败。

Codex 插件清单读取失败时保持 fail-closed：诊断不会宣称仓库是干净的，而是报告"无法排除以插件形式安装的框架"，并附上失败原因，同时仍返回 1。
